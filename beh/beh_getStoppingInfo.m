function [stopSignalBeh, RTdist] = beh_getStoppingInfo(stateFlags,Infos,ttx)
...
RTdist.nostop = util_cumulDist(RTdist.all(ttx.nostop.all.all));
RTdist.noncanc = util_cumulDist(RTdist.all(ttx.noncanceled.all.all));
...
[~,stopSignalBeh.midSSDidx] = min(abs(stopSignalBeh.inh_pnc-0.5));;

[stopSignalBeh.inh_weibull.parameters,~,...
    stopSignalBeh.inh_weibull.x,...
    stopSignalBeh.inh_weibull.y] =...
    util_fitWeibull(stopSignalBeh.inh_SSD, stopSignalBeh.inh_pnc, stopSignalBeh.inh_nTr);

stopSignalBeh.ssrt = beh_getSSRT(stopSignalBeh.inh_SSD,...
